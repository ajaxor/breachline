...
export class TargetingPolicy {
  ...
  canTarget(attacker, target, type = UNIT_TYPES[attacker.type]) {
    ...
    if (hasUnitTag(type, UNIT_TAG.RAM) && targetType?.role === UNIT_ROLE.WALL) return false;
    if (targetType?.role === UNIT_ROLE.WALL && !this.isBlockingAdjacent(attacker, target)) return false;
