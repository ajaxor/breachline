...
function addDeathEffect(model, unit, at, duration, actionStart) {
  ...
  model.effects.push({ type: EFFECT_TYPE.DEATH, ...point(unit), shape: definition.shape, graphic: definition.graphic, deathStyle: definition.animation.death, color: teamColor(unit.team), seed: unit.id * 2.399963229728653, actionStart, start: at, duration: Math.max(duration * 1.25, 450) });
}
function addGroundDeathExplosion(model, unit, at, duration) { model.effects.push({ type: EFFECT_TYPE.EXPLOSION, ...point(unit), start: at, duration: Math.max(duration, 320), intensity: 0.9 }); }
...
export class CombatEventPresenter {
  ...
  actionTime(event) { const at = event.at ?? 0; if (this.sequenceTickAt !== at) { this.sequenceTickAt = at; this.sequenceIndex = -1; this.currentActionAt = at; } const isAttack = event.type === COMBAT_EVENT.UNIT_ATTACKED || event.type === COMBAT_EVENT.UNIT_DODGED || event.type === COMBAT_EVENT.UNIT_PUSHED; const attacker = event.attacker ?? event.source; const isMelee = isAttack && UNIT_TYPES[attacker.type].animation.attack === ATTACK_ANIMATION.MELEE; if (isMelee) return at; if (isAttack || [COMBAT_EVENT.UNIT_HEALED, COMBAT_EVENT.BASE_ATTACKED].includes(event.type)) { this.sequenceIndex += 1; this.currentActionAt = at + this.sequenceIndex * ATTACK_STAGGER_MS; } return this.currentActionAt; }
  present(model, event) {
    ...
    switch (event.type) {
      ...
      case COMBAT_EVENT.UNIT_PUSHED: addAttackEffect(model, event.source, event.target, at, duration); model.addLog(`${UNIT_TYPES[event.source.type].name} #${event.source.id} pushes ${UNIT_TYPES[event.unit.type].name} #${event.unit.id} backward.`, LOG_TYPE.HIT); break;
      case COMBAT_EVENT.SPLASH_HIT: { const impactAt = at + duration * 0.2; model.effects.push({ type: EFFECT_TYPE.TEXT, ...point(event.target), text: `-${event.damage}`, color: '#ff5d5d', actionStart: at, start: impactAt, duration: duration * 1.3 }); addHealthLossEffect(model, event.target, event.damage, at, impactAt, duration); model.addLog(`Splash blast hits ${UNIT_TYPES[event.target.type].name} #${event.target.id} for ${event.damage}.`, LOG_TYPE.HIT); break; }
      ...
      case COMBAT_EVENT.UNIT_BREACHED: model.effects.push({ type: EFFECT_TYPE.TEXT, ...point(event.unit), text: 'BREACH!', color: '#fbbf24', start: at, duration: Math.max(duration * 1.6, 400) }); model.addLog(`${UNIT_TYPES[event.unit.type].name} #${event.unit.id} breaks through and begins sieging the ${event.targetBase} base!`, LOG_TYPE.SYSTEM); break;
      case COMBAT_EVENT.BASE_ATTACKED: model.effects.push({ type: EFFECT_TYPE.TEXT, ...point(event.unit), text: `-${event.damage}`, color: '#fbbf24', start: at, duration: duration * 1.2 }); model.addLog(`${UNIT_TYPES[event.unit.type].name} #${event.unit.id} strikes the ${event.targetBase} base for ${event.damage}.`, event.unit.team === TEAM.PLAYER ? LOG_TYPE.KILL : LOG_TYPE.PLAYER_LOSS); break;
      case COMBAT_EVENT.UNIT_DESTROYED: { const definition = UNIT_TYPES[event.unit.type]; const healthLoss = latestHealthLoss(model, event.unit.id); const effectAt = healthLoss?.start ?? at; if (definition.animation.death === DEATH_ANIMATION.EXPLODE) addGroundDeathExplosion(model, event.unit, effectAt, duration); else addDeathEffect(model, event.unit, effectAt, duration, healthLoss?.actionStart); if (!event.silent) model.addLog(`${definition.name} #${event.unit.id} destroyed.`, event.unit.team === TEAM.PLAYER ? LOG_TYPE.PLAYER_LOSS : LOG_TYPE.KILL); break; }
      case COMBAT_EVENT.BATTLE_FINISHED: model.addLog(event.result.text, LOG_TYPE.SYSTEM); break;
