...
function addDeathEffect(model, unit, at, duration, actionStart) {
  ...
  model.effects.push({ type: EFFECT_TYPE.DEATH, ...point(unit), shape: definition.shape, graphic: definition.graphic, deathStyle: definition.animation.death, color: teamColor(unit.team), seed: unit.id * 2.399963229728653, actionStart, start: at, duration: Math.max(duration * 2.1, 760) });
}
function addGroundDeathExplosion(model, unit, at, duration) { model.effects.push({ type: EFFECT_TYPE.EXPLOSION, ...point(unit), start: at, duration: Math.max(duration * 1.55, 560), intensity: 0.9 }); }
