...
export const drawUnitGraphic = (context, graphic, x, y, radius, color, role = null) => {
  ...
  if (silhouetteRole === 'flying') {
    context.save();
    context.translate(0, radius * 0.14);
...
function drawGruntSymbol(ctx, radius) { fillRectScaled(ctx, radius, -0.42, -0.48, 0.84, 0.96); fillRectScaled(ctx, radius, -0.56, -0.08, 1.12, 0.16); }
...
function drawSwarmerSymbol(ctx, radius) { ctx.beginPath(); [[-0.34, -0.22], [0.18, -0.3], [-0.08, 0.18], [0.38, 0.16], [-0.36, 0.38]].forEach(([x, y]) => { ctx.moveTo(x * radius + radius * 0.13, y * radius); ctx.arc(x * radius, y * radius, radius * 0.13, 0, Math.PI * 2); }); ctx.fill(); }
...
function drawFusilierSymbol(ctx, radius) { ctx.lineWidth = Math.max(1.5, radius * 0.11); ctx.beginPath(); [-0.28, 0, 0.28].forEach((y, i) => { ctx.moveTo(-radius * 0.46, radius * y); ctx.lineTo(0, radius * (y + 0.2)); ctx.lineTo(radius * 0.46, radius * y); }); ctx.stroke(); }
...
function drawMidgeSymbol(ctx, radius) { fillShape(ctx, radius, [[0, -0.62], [0.13, -0.16], [0.08, 0.58], [-0.08, 0.58], [-0.13, -0.16]]); fillShape(ctx, radius, [[-0.12, -0.18], [-0.62, -0.42], [-0.34, 0.08]]); fillShape(ctx, radius, [[0.12, -0.18], [0.62, -0.42], [0.34, 0.08]]); }
function drawWaspSymbol(ctx, radius) { fillShape(ctx, radius, [[0, -0.74], [0.18, -0.22], [0.1, 0.36], [0, 0.78], [-0.1, 0.36], [-0.18, -0.22]]); fillShape(ctx, radius, [[-0.16, -0.18], [-0.7, -0.38], [-0.34, 0.08]]); fillShape(ctx, radius, [[0.16, -0.18], [0.7, -0.38], [0.34, 0.08]]); fillShape(ctx, radius, [[0, 0.46], [0.26, 0.7], [0.08, 0.78]]); fillShape(ctx, radius, [[0, 0.46], [-0.26, 0.7], [-0.08, 0.78]]); }
...
function drawFireflySymbol(ctx, radius) { ctx.lineWidth = Math.max(1.4, radius * 0.1); ctx.beginPath(); ctx.arc(0, 0, radius * 0.24, 0, Math.PI * 2); ctx.fill(); for (let i = 0; i < 6; i += 1) { const angle = -Math.PI / 2 + i * Math.PI / 3; ctx.moveTo(Math.cos(angle) * radius * 0.36, Math.sin(angle) * radius * 0.36); ctx.lineTo(Math.cos(angle) * radius * 0.62, Math.sin(angle) * radius * 0.62); } ctx.stroke(); }
...
function drawDemolisherSymbol(ctx, radius) { ctx.lineWidth = Math.max(1.4, radius * 0.1); ctx.beginPath(); ctx.arc(0, radius * 0.08, radius * 0.38, 0, Math.PI * 2); ctx.stroke(); fillShape(ctx, radius, [[-0.14, -0.34], [0.08, -0.64], [0.24, -0.3]]); fillShape(ctx, radius, [[0.28, -0.5], [0.48, -0.7], [0.5, -0.4]]); }
