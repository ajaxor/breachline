...
const ABSTRACT_UNIT_DRAWERS = Object.freeze({
  ...
  'tesla-coil': drawTeslaCoilSymbol,
  factory: drawFactorySymbol,
  hangar: drawHangarSymbol,
});
...
function drawReaperSymbol(ctx, radius) { ctx.lineWidth = Math.max(1.6, radius * 0.12); ctx.beginPath(); ctx.arc(-radius * 0.08, -radius * 0.08, radius * 0.52, -Math.PI * 0.76, Math.PI * 0.2); ctx.stroke(); fillShape(ctx, radius, [[-0.48, -0.52], [0.48, -0.3], [0.18, -0.04]]); ctx.beginPath(); ctx.moveTo(-radius * 0.12, -radius * 0.02); ctx.lineTo(radius * 0.38, radius * 0.58); ctx.stroke(); }
function drawPikeSymbol(ctx, radius) { fillShape(ctx, radius, [[0, -0.72], [0.16, -0.4], [-0.16, -0.4]]); fillRectScaled(ctx, radius, -0.07, -0.4, 0.14, 1.08); fillShape(ctx, radius, [[-0.54, -0.12], [-0.18, -0.34], [-0.18, 0.4], [-0.54, 0.22]]); fillShape(ctx, radius, [[0.54, -0.12], [0.18, -0.34], [0.18, 0.4], [0.54, 0.22]]); }
function drawGunnerSymbol(ctx, radius) { ctx.lineWidth = Math.max(1.6, radius * 0.13); ctx.beginPath(); ctx.arc(-radius * 0.12, radius * 0.08, radius * 0.28, 0, Math.PI * 2); ctx.stroke(); ctx.beginPath(); ctx.moveTo(radius * 0.06, -radius * 0.18); ctx.lineTo(radius * 0.62, -radius * 0.4); ctx.moveTo(radius * 0.1, radius * 0.02); ctx.lineTo(radius * 0.7, -radius * 0.12); ctx.stroke(); fillShape(ctx, radius, [[-0.62, -0.2], [-0.18, -0.06], [-0.28, 0.18], [-0.68, 0.04]]); ctx.beginPath(); ctx.arc(-radius * 0.12, radius * 0.08, radius * 0.08, 0, Math.PI * 2); ctx.fill(); }
...
function drawMidgeSymbol(ctx, radius) { ctx.beginPath(); ctx.arc(0, 0, radius * 0.2, 0, Math.PI * 2); ctx.fill(); fillShape(ctx, radius, [[-0.18, 0.18], [-0.46, 0.42], [-0.1, 0.36]]); fillShape(ctx, radius, [[0.18, 0.18], [0.46, 0.42], [0.1, 0.36]]); }
...
function drawFireflySymbol(ctx, radius) { ctx.beginPath(); ctx.arc(0, radius * 0.22, radius * 0.26, 0, Math.PI * 2); ctx.fill(); fillRectScaled(ctx, radius, -0.08, -0.18, 0.16, 0.24); fillShape(ctx, radius, [[-0.02, -0.2], [0.2, -0.46], [0.32, -0.24]]); }
...
function drawMineSymbol(ctx, radius) { ctx.beginPath(); ctx.arc(0, radius * 0.08, radius * 0.28, 0, Math.PI * 2); ctx.fill(); [[0, -0.62], [0.5, -0.26], [0.58, 0.3], [0, 0.7], [-0.58, 0.3], [-0.5, -0.26]].forEach(([x, y]) => fillShape(ctx, radius, [[x * 0.48, y * 0.48], [x, y], [x * 0.7 - y * 0.1, y * 0.7 + x * 0.1]])); }
...
function drawTeslaCoilSymbol(ctx, radius) { ctx.lineWidth = Math.max(1.5, radius * 0.11); fillRectScaled(ctx, radius, -0.48, 0.4, 0.96, 0.16); fillRectScaled(ctx, radius, -0.08, -0.06, 0.16, 0.48); ctx.beginPath(); ctx.arc(0, -radius * 0.18, radius * 0.18, 0, Math.PI * 2); ctx.stroke(); ctx.beginPath(); ctx.arc(0, -radius * 0.18, radius * 0.38, -Math.PI * 0.82, -Math.PI * 0.18); ctx.arc(0, -radius * 0.18, radius * 0.58, -Math.PI * 0.78, -Math.PI * 0.22); ctx.stroke(); }
...
function drawHangarSymbol(ctx, radius) { fillRectScaled(ctx, radius, -0.62, 0.3, 1.24, 0.2); fillShape(ctx, radius, [[-0.58, 0.28], [-0.42, -0.34], [0.42, -0.34], [0.58, 0.28]]); ctx.save(); ctx.globalCompositeOperation = 'destination-out'; fillRectScaled(ctx, radius, -0.24, -0.04, 0.48, 0.34); ctx.restore(); fillShape(ctx, radius, [[0, -0.5], [0.34, -0.12], [0.08, -0.18], [0, 0.02], [-0.08, -0.18], [-0.34, -0.12]]); }
